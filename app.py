...
MODEL_FILENAME = 'XGBoost_BikeSharing_Final_Model_PyCaret.pkl'
...
def run_prediction_app():
    ...
    col_date, col_time = st.columns([1, 1]) # Rasio kolom bisa disesuaikan
    with col_date:
    ...
    # Informasi hari kerja berdasarkan tanggal yang dipilih
    is_working_day_auto = 1 if dt_object.weekday() < 5 else 0 # Senin (0) - Jumat (4) adalah hari kerja
    ...
    # --- Bagian Input Fitur Lainnya dengan Kolom ---
    st.markdown("#### 📋 Kondisi & Lingkungan")
    # Kita akan menggunakan 3 kolom untuk input ini
    col_kondisi1, col_kondisi2, col_lingkungan = st.columns(3)

    with col_kondisi1: # Kolom untuk Musim dan Hari Libur
    ...
    with col_kondisi2: # Kolom untuk Hari Kerja dan Cuaca
    ...
    with col_lingkungan: # Kolom untuk Suhu, Kelembapan, Kecepatan Angin
...
def show_model_info_page():
    ...
    st.markdown(f"""
    Model prediktif yang menjadi tulang punggung aplikasi ini adalah **XGBoost Regressor**.
    Model ini telah dilatih dan dioptimalkan menggunakan _framework_ PyCaret.

    #### Arsitektur & Pra-pemrosesan:
    Model yang Anda gunakan (`{MODEL_FILENAME}`) bukan hanya sekadar algoritma XGBoost, melainkan **keseluruhan pipeline pra-pemrosesan dan model** yang telah di-*fit* pada data historis. Proses ini mencakup berbagai langkah seperti ekstraksi fitur waktu, rekayasa fitur siklikal, penanganan pencilan (winsorizing), scaling, dan encoding fitur kategorikal, serta transformasi target.

    #### Sumber Data Acuan:
    Model ini dikembangkan berdasarkan konsep dan data dari kompetisi Kaggle:
    [Bike Sharing Demand - Kaggle](https://www.kaggle.com/competitions/bike-sharing-demand/data)

    #### Performa Model (Contoh dari Sesi Pelatihan):
    ...
    *Metrik ini menunjukkan kemampuan prediksi model, namun performa pada data baru dapat bervariasi.*
    ...
    if pipeline_model is not None:
        st.markdown("#### Parameter Estimator Inti (XGBoost):")
        try:
            actual_model_estimator = pipeline_model.steps[-1][1]
            if hasattr(actual_model_estimator, 'regressor'): 
                actual_model_estimator = actual_model_estimator.regressor
...
if __name__ == "__main__":
    if pipeline_model is None:
        ...
        st.markdown("Pastikan file model `XGBoost_BikeSharing_Final_Model_PyCaret.pkl` ada di direktori yang sama dengan `app.py`.")
        st.stop()
    main()